...
module MK_FSM(
    ...
    // разрядность принимаемых данных 108 битов, т.е. 27 символов / кол-во принимаемых полубайтов => разрядность DATA_CT = 5
    ...
    reg [4:0] DATA_CT; // upd
    reg [4:0] RES_CT; // upd
    reg [107:0] DATA_REG; // upd
    reg [123:0] RES_REG; // upd
    reg [6:0] END_ADDR;
    reg RES_FLG;
    localparam [3:0] IDLE = 4'd0;
    localparam [3:0] RDT = 4'd1;
    localparam [3:0] RCR = 4'd2;
    localparam [3:0] RLF = 4'd3;
    localparam [3:0] TRES = 4'd4;
    localparam [3:0] TMEM = 4'd5;
    localparam [3:0] TDT = 4'd6;
    localparam [3:0] TCR = 4'd7;
    localparam [3:0] TLF = 4'd8;
    localparam [6:0] RES_A0 = 7'b0000000; // начальный адрес сообщения
    localparam [6:0] RES_A1 = 7'b0000110; // конечный адрес сообщения
    // адреса ошибок
    reg [6:0] ERR_A0_MX;
    reg [6:0] ERR_A1_MX;
    ...
    always @*
        case (RX_DATA_R[9:8]) // upd
            2'b00: begin // error format data
                ERR_A0_MX <= 7'b0000111;
                ERR_A1_MX <= 7'b0010101;
            end
            2'b01: begin // error parity | ошибка четности
                ERR_A0_MX <= 7'b0010110;
                ERR_A1_MX <= 7'b0100111;
            end
            2'b01: begin // error format | ошибка формата кадра
                ERR_A0_MX <= 7'b0101000;
                ERR_A1_MX <= 7'b0111100;
            end
            default: begin // 2'b11 | error parity and format | ошибка формата и четности
                ERR_A0_MX <= 7'b0111101;
                ERR_A1_MX <= 7'b1000100;
            end
        endcase
